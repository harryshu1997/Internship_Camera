...
public class CameraSession extends Session {
    ...
    public void applyRequest(int msg, Object value1, Object value2) {
        switch (msg) {
            ...
            case RQ_ZOOM:{ //add 2021/6/9
                sendZoomRequest((float)value1);
                break;
            }
            case RQ_FOCUS_LENGTH:{ //add 2021/7/1
                sendZoomRequest((float)value1);
                break;
            }
            default: {
    ...
    //add 2021/6/9 zoom
    private void sendZoomRequest(float v){
        Log.d(TAG, "set zoom value: " + v);
        CaptureRequest request = mRequestMgr.getZoomRequest(getPreviewBuilder(),v);
    ...
    private void sendStillPictureRequest() {
        ...
        //add Robotrak picture quality settings
        builder.set(CaptureRequest.SENSOR_SENSITIVITY,getPreviewBuilder().get(CaptureRequest.SENSOR_SENSITIVITY));
        builder.set(CaptureRequest.SENSOR_EXPOSURE_TIME,getPreviewBuilder().get(CaptureRequest.SENSOR_EXPOSURE_TIME));
       // builder.set(CaptureRequest.SENSOR_FRAME_DURATION,getPreviewBuilder().get(CaptureRequest.SENSOR_FRAME_DURATION));
        //set zoom value
        builder.set(CaptureRequest.SCALER_CROP_REGION,getPreviewBuilder().get(CaptureRequest.SCALER_CROP_REGION));
        builder.set(CaptureRequest.LENS_FOCAL_LENGTH,getPreviewBuilder().get(CaptureRequest.LENS_FOCAL_LENGTH));
       ...
       // builder.set(CaptureRequest.CONTROL_AF_MODE, afMode); //2021/7/7
    ...
    private void sendControlFocusLengthRequest(float value) {
        CaptureRequest request = mRequestMgr.getFocusLengthRequest(getPreviewBuilder(), value);
        sendRepeatingRequest(request, mPreviewCallback, mMainHandler);
    }
    ...
    private void sendControlSensitivityRequest(int value){
        ...
        sendRepeatingRequest(request, mPreviewCallback, mMainHandler);
        Log.i(TAG,"set sensitivity to: " + value);
        System.out.println("sense: " + value);
...
//        mRequestMgr.getAEModeRequest(getPreviewBuilder(), CameraCharacteristics.CONTROL_AE_MODE_ON);
//
...
//        mRequestMgr.getExposureTimeRequest(getPreviewBuilder(),val); 
        ...
        long EXP = g.getExpTime();
        long Frame = g.getFrameDuration();
        int Sense = g.getSensitivity();
        mRequestMgr.getExposureTimeRequest(getPreviewBuilder(),EXP);
        Log.i(TAG,"set EXP VAL to: " + EXP);
        mRequestMgr.getFrameDurationRequest(getPreviewBuilder(),Frame);
        Log.i(TAG,"set FrameDuration VAL to: " + Frame);
        mRequestMgr.getSensitivityRequest(getPreviewBuilder(),Sense);
        Log.i(TAG,"set Sensitivity VAL to: " + Sense);
